  ...
  const renderStep = () => {
    ...
    if (step === 2) return (
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-3"><Switch checked={data.skipOneThird} onCheckedChange={(v) => update({ skipOneThird: v })} /><Label>Não calcular 1/3 de férias</Label></div>
    ...
    return (
      <div className="space-y-4">
        ...
        <Card><CardContent className="pt-4 text-sm space-y-1"><p><strong>Memória:</strong></p><p>RB = {formatCurrency(data.salaryBase)} + {formatCurrency(data.avgVariables)} + {formatCurrency(data.otherPayItems)} = {formatCurrency(result.baseRemuneration)}</p><p>VF = RB ÷ 30 × {result.effectiveLeaveDays} (dias de gozo, já descontado o abono) = {formatCurrency(result.vacationValue)}</p>{!data.skipOneThird && <p>T = VF ÷ 3 = {formatCurrency(result.oneThirdValue)}</p>}{data.abonoEnabled && <p>VA = RB ÷ 30 × {data.abonoDays} = {formatCurrency(result.abonoValue)}; TA = VA ÷ 3 = {formatCurrency(result.abonoOneThirdValue)}</p>}<p>Líquido = {formatCurrency(result.grossTotal)} - {formatCurrency(data.discountsValue)} = {formatCurrency(result.netTotal)}</p></CardContent></Card>
